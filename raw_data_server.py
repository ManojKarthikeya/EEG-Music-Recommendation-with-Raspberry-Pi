...
def fetch_headset_data():
    global val_data
    while True:
        eeg_device.fetch_data()
        if "eeg_asic" in eeg_device.data.keys():
            print(eeg_device.data)
            val_data = eeg_device.data.copy()

def run_server():
    app.run()
    
if __name__ =="__main__":
    t1 = threading.Thread(target=fetch_headset_data)
