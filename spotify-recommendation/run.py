...
def get_random_song(sp: Spotify):
    while True:
        ...
        logging.info(f"Attempting to search songs with search_term={search_term} and offset={random_idx}...")
        search_results = sp.search(q=search_term, limit=50, offset=random_idx, type="track", market="US")
        logging.info(f"Successfully found search results with search_term={search_term}!")

        tracks = search_results["tracks"]["items"]
        random.shuffle(tracks)

        logging.info(f"Attempting to find songs in search results with preview_url...")
        picked_track = None
        for track in tracks:
            if track["preview_url"] is not None:
                ...
                logging.info(f"No preview url for track ({track['name']})... skipping track")
        
        if picked_track is not None:
            break
        else:
            logging.info(f"Failed to find songs in search results with preview_url... Will attempt with new search term")
            continue
...
def get_recommendations(sp: Spotify, tracks: list):
    recommendations = sp.recommendations(seed_tracks=tracks, limit=len(tracks)*2, country="US")
...
def main():
    ...
    for _ in range(10):
        ...
        for i in tqdm(range(14)):
            ...
            brain_waves.append([data['delta'], data['theta'], data['low-alpha'], data['high-alpha'], data['low-beta'], data["high-beta"], data["low-gamma"], data["mid-gamma"], raw_data["attention"], raw_data["meditation"]])
    ...
    if len(positive) < 5:
        logging.info(f"Not enough positve tracks found ({len(positive)})... appending neutral tracks ({len(neutral)}) also to shortlist!")
